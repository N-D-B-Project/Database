...
export class InitialTypeORMSchema1776818540402 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TABLE "users" ("created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "id" character varying NOT NULL, "database_version" character varying NOT NULL DEFAULT 'API', CONSTRAINT "pk_users" PRIMARY KEY ("id"))`,
		);
		await queryRunner.query(
			`CREATE INDEX "idx_users_created_at" ON "users" ("created_at") `,
		);
		await queryRunner.query(
			`CREATE TABLE "user_settings" ("created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "id" uuid NOT NULL DEFAULT uuid_generate_v4(), "premium" boolean NOT NULL DEFAULT false, "user_id" character varying NOT NULL, CONSTRAINT "uq_user_settings_user_id" UNIQUE ("user_id"), CONSTRAINT "REL_4ed056b9344e6f7d8d46ec4b30" UNIQUE ("user_id"), CONSTRAINT "pk_user_settings" PRIMARY KEY ("id"))`,
		);
		await queryRunner.query(
			`CREATE INDEX "idx_user_settings_user_id" ON "user_settings" ("user_id") `,
		);
		await queryRunner.query(
			`ALTER TABLE "api_users" ADD CONSTRAINT "fk_api_users_user_id" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "guild_reaction_roles" ADD CONSTRAINT "fk_guild_reaction_roles_guild_id" FOREIGN KEY ("guild_id") REFERENCES "guilds"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "guild_settings" ADD CONSTRAINT "fk_guild_settings_guild_id" FOREIGN KEY ("guild_id") REFERENCES "guilds"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "ticket_types" ADD CONSTRAINT "fk_ticket_types_guild_settings_id" FOREIGN KEY ("guild_settings_id") REFERENCES "guild_settings"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "tickets" ADD CONSTRAINT "fk_tickets_user_id" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "tickets" ADD CONSTRAINT "fk_tickets_guild_id" FOREIGN KEY ("guild_id") REFERENCES "guilds"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "tickets" ADD CONSTRAINT "fk_tickets_ticket_type_id" FOREIGN KEY ("ticket_type_id") REFERENCES "ticket_types"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
		);
		await queryRunner.query(
			`ALTER TABLE "user_settings" ADD CONSTRAINT "fk_user_settings_user_id" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);
	}
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "tickets" DROP CONSTRAINT "fk_tickets_guild_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "tickets" DROP CONSTRAINT "fk_tickets_user_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "ticket_types" DROP CONSTRAINT "fk_ticket_types_guild_settings_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "guild_settings" DROP CONSTRAINT "fk_guild_settings_guild_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "guild_reaction_roles" DROP CONSTRAINT "fk_guild_reaction_roles_guild_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "api_users" DROP CONSTRAINT "fk_api_users_user_id"`,
		);
		...
		await queryRunner.query(
			`DROP INDEX "public"."idx_ticket_types_guild_settings_id"`,
		);
		await queryRunner.query(`DROP TABLE "ticket_types"`);
		await queryRunner.query(
			`DROP INDEX "public"."idx_guild_settings_guild_id"`,
		);
		await queryRunner.query(`DROP TABLE "guild_settings"`);
		await queryRunner.query(
			`DROP INDEX "public"."idx_guild_reaction_roles_guild_id"`,
		);
		await queryRunner.query(
			`DROP INDEX "public"."idx_guild_reaction_roles_message"`,
		);
